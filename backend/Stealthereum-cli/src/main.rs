...
fn main() {
    let cli = Cli::parse();
    match &cli.command {
        Some(Commands::Keygen { output }) => match output {
            Some(output) => {
                let _ = keygen(output);
            }
            None => {
        ...
        Some(Commands::RevealStealthKeyNoFile {
            spendingkey,
            viewingkey,
            stealthaddr,
            ephemeralpub,
